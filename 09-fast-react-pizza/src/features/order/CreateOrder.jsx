  ...
  return (
    <div className="px-4 py-6">
      ...
      <Form method="POST" action="/order/new">
        ...
        {formErrors?.phone && <p className="mt-2 mb-5 rounded-md bg-red-200 p-2 text-xs text-red-700">{formErrors.phone}</p>}
        ...
        {addressStatus  === 'error'  && <p className="mt-2 mb-5 rounded-md bg-red-200 p-2 text-xs text-red-700">{errorAddress}</p>}
         
          </div>
          
          { !position.latitude && !position.longitude && (<span  className="absolute right-[3px] top-[3px] z-50  md:top-[5px]">  <Button type="small" disabled={isLoadingAddress} onClick={(e)=>{e.preventDefault(); dispatch(fetchAddress())}}>Get position</Button></span>)}
...
export async function action({ request }) {
  ...
  //if eberything is okay,create new order and redirect
